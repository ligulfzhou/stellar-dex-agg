...
export type BuildOrderTxResult = BuildTxResult & { contract: string };
...
export class LumAggClient {
  ...
  async getBalances(params: { account: string }): Promise<BalancesResult> {
    ...
    return {
      ...
      tokensQueried: Array.isArray(json.tokens_queried)
        ...
        : [],
      balances: (json.balances ?? {}) as Record<string, string>,
      hasTrustline: (json.has_trustline ?? {}) as Record<string, boolean>,
      updatedAtMs: Number(json.updated_at_ms ?? 0),
    };
  }

  async getAccount(params: { account: string }): Promise<AccountResult> {
    const search = new URLSearchParams({ account: params.account });
  ...
  async getTxStatus(params: { hash: string }): Promise<TxStatusResult> {
